...
import { User, Mail, Lock, Phone, ArrowUpRight, ShieldAlert, KeyRound, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function RegisterPage({ onSuccess, onGoToLogin }: RegisterProps) {
  const { requestRegisterOtp, verifyRegisterOtp, resendAuthOtp } = useApp();
  ...
  const handleSubmitDetails = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !email || !password || !phone) return;
    setError('');
    setNotice('');
    setLoading(true);

    const res = await requestRegisterOtp(email, password, fullName, phone);
    setLoading(false);
  ...
  const handleResend = async () => {
    setError('');
    setResending(true);
    const res = await resendAuthOtp(email, 'register');
    setResending(false);
    if (res.success) {
      setNotice(
        res.devCode
    ...
    <div className="w-full min-h-[70vh] bg-stone-50 flex items-center justify-center p-6 py-12">
      <div className="w-full max-w-md bg-white border border-stone-150 rounded-3xl p-8 shadow-xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 id="register-heading" className="text-2xl md:text-3xl font-serif italic text-emerald-950 font-bold uppercase tracking-wide">
            Create Guest Profile
          </h2>
          <p className="text-xs text-stone-500 font-mono">
            {step === 'details'
              ? 'Register below to build customized safaris and coordinate with expert trackers.'
        ...
        {/* Error Alert Display */}
        {error && (
          <div id="register-error-alert" className="p-3 bg-red-50 text-red-700 text-xs font-mono rounded-xl border border-red-100 flex items-start gap-2 animate-shake">
        ...
        {notice && !error && (
          <div id="register-otp-notice" className="p-3 bg-emerald-50 text-emerald-800 text-xs font-mono rounded-xl border border-emerald-100 flex items-start gap-2">
        ...
        {step === 'details' && (
          <form onSubmit={handleSubmitDetails} className="space-y-4 text-xs font-mono">
        ...
        {step === 'otp' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4 text-xs font-mono">
            ...
            <div className="flex items-center justify-between pt-1">
              <button
                type="button"
                id="register-back-to-details-btn"
                onClick={() => {
                  setStep('details');
                  setOtpCode('');
                  setError('');
